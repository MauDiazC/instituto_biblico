  ...
  const fetchClassData = async (silent = false) => {
    try {
      ...
      if (classData.video_url) {
        ...
        // Fetch temporary link if it's recorded but no direct URL
      ...
      const mainChannel = supabase
        ...
        .on('postgres_changes', { 
          ...
          if (payload.new && payload.new.id === parseInt(lessonId)) {
            console.log('REALTIME: Local class update detected', payload.new.status);
            fetchClassData(true);
          }
        })
        .on('postgres_changes', { 
        ...
        .subscribe((status) => {
          console.log(`REALTIME: Channel status for ${lessonId}: ${status}`);
        });

      // Polling Fallback (Every 5 seconds) - very fast and reliable
  ...
  return (
    <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 pb-12">
      <div className="lg:col-span-8 space-y-6">
        {/* Force fresh render of the video area on status change */}
          ...
          {clase.status === 'LIVE' && clase.room_url ? (
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-white bg-black/60 text-center p-8">
              ...
              <h3 className="text-2xl font-black font-headline mb-2">
                {clase.status === 'PLANNED' ? 'Sesión Programada' : 'La transmisión terminó'}
              </h3>
              <p className="text-white/60 max-w-xs mx-auto font-body">
                {clase.status === 'PLANNED' ? 'Espera a que el tutor inicie la clase. Esta página se actualizará sola.' : 'El video de la sesión estará disponible en unos minutos.'}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {isTeacher && (clase.status === 'LIVE' || clase.status === 'PLANNED') && (
                <button 
                  ...
                  disabled={isEnding}
                  className="px-6 py-3 bg-error text-white rounded-lg font-headline font-bold transition-all shadow-sm flex items-center gap-2 active:scale-95"
                >
              ...
              >
                ...
                {clase.is_completed ? 'Hecha' : 'Completar'}
              </button>
            </div>
          </div>
          ...
          <div className="p-6 bg-surface-container-lowest rounded-xl shadow-sm border-l-4 border-secondary">
            <p className="text-on-surface leading-relaxed text-lg font-body">
              {clase.status === 'LIVE' ? "Participa en la clase en vivo y haz tus preguntas." : 
               clase.status === 'PLANNED' ? "La clase aún no ha comenzado. Prepárate." :
               "Lección finalizada. Puedes volver a ver el video."}
            </p>
          </div>
                ...
                <div className="space-y-3">
                  ...
                  {userSubmission ? (
                    <div className="bg-green-50 p-4 rounded-xl border border-green-100">
                      <p className="text-xs font-black text-green-700 uppercase flex items-center gap-2 mb-2"><CheckCircle2 className="w-4 h-4" /> Entregada</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      <textarea value={submission} onChange={(e) => setSubmission(e.target.value)} placeholder="Tu respuesta..." className="w-full p-3 text-xs bg-white border border-outline-variant/20 rounded-xl focus:ring-2 focus:ring-secondary outline-none min-h-[80px]" />
                      <button onClick={() => handleSubmitTarea(tarea.id)} disabled={isSubmitting} className="w-full bg-secondary text-white py-3 rounded-xl font-black text-xs uppercase hover:bg-on-secondary-container transition-all">
                        {isSubmitting ? '...' : 'Entregar Tarea'}
                      </button>
                    </div>
                  )}
                </div>
      ...
      <div className="lg:col-span-4 space-y-6">
        <div className="bg-surface-container-low rounded-xl p-6 sticky top-28 shadow-sm">
          ...
          <div className="space-y-3">
            <div className={`p-4 rounded-xl shadow-md flex items-center gap-4 transition-all duration-500 ${clase.status === 'LIVE' ? 'bg-error text-white ring-4 ring-error/20' : 'bg-primary text-white opacity-80'}`}>
              <div className="flex-1">
                <p className="text-sm font-bold font-headline">{clase.title}</p>
                <p className="text-[10px] opacity-70 font-label uppercase font-black">{clase.status === 'LIVE' ? 'EN VIVO AHORA' : clase.status}</p>
              </div>
              {clase.status === 'LIVE' ? (
          ...
          <div className="mt-8 border-t border-outline-variant/10 pt-8 space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="font-black text-primary font-headline uppercase tracking-tight text-xs flex items-center gap-2"><HelpCircle className="w-4 h-4 text-secondary" /> Preguntas</h3>
